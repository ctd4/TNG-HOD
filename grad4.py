...
for n in range(0,Sub_effective_rad.shape[0]):
	...
	cutout_request = {'stars':'Coordinates,GFM_StellarFormationTime'}
	...
	with h5py.File(cutout,'r') as f:
		...
		gasmet=f['PartType4']['GFM_StellarFormationTime'][:]
...
#First Bin
	...
	bins=np.arange(0,2*Sub_effective_rad[n],(2*Sub_effective_rad[n])/10)
	metallicity_means1=[]
	distance_bin1=[]
	met_grad=[]
	for i in range(1,len(bins)):
		...
		if distances[mask].shape[0]<10:
			pass
		else:
			distance_bin1.append(np.amax(distances[mask])-(2*Sub_effective_rad[n])/20)
			metallicity_means1.append(np.mean(gasmet[mask]))
#Second bin
	bins=np.arange(2*Sub_effective_rad[n],4*Sub_effective_rad[n],(2*Sub_effective_rad[n])/10)
	metallicity_means2=[]
	distance_bin2=[]
	met_grad=[]
	for i in range(1,len(bins)):
		...
		if distances[mask].shape[0]<10:
			...
			distance_bin2.append(np.amax(distances[mask])-(2*Sub_effective_rad[n])/20)
			metallicity_means2.append(np.mean(gasmet[mask]))

#Write to Files
	with open("age_grad5.csv",'a') as f:
		writer = csv.writer(f)
		if not metallicity_means1:
			writer.writerow(str(0))
		else:
			writer.writerow([str(np.polyfit(np.array(distance_bin1),np.array(metallicity_means1),1)[0])])

		
	with open("age_grad6.csv",'a') as f:
		writer = csv.writer(f)
		if not metallicity_means2:
			writer.writerow(str(0))
		else:
			writer.writerow([str(np.polyfit(np.array(distance_bin2),np.array(metallicity_means2),1)[0])])
